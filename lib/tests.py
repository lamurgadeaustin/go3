...
class CaldavTest(TestCase):
    ...
    def create_gig(self):
        the_date = timezone.datetime(
            ...
            hour=14,
            minute=30,
            tzinfo=pytz.timezone(self.band.timezone),
        )
    ...
    def test_calfeed(self):
        cf = make_calfeed(
            "flim-flam", [], self.joeuser.preferences.language, self.joeuser.cal_feed_id
        )
    ...
    def test_calfeed_translation(self):
        ...
        self.testgig.save()
        cf = make_calfeed(
            b"flim-flam", self.band.gigs.all(), "de", self.joeuser.cal_feed_id
        )
